...
| 요소 | 현재 규칙 |
|---|---|
| 특수 지형 | 일반 칸마다 총 0.5% (추출의 성소 1, 상점 0.5, 건강의 성소 0.5 비중) |
| 상점 | 특수 지형 중 0.125% |
| 추출의 성소 | 특수 지형 중 0.25% |
| 건강의 성소 | 특수 지형 중 0.125% (최대 체력 +5, 현재 체력 변화 없음) |
...
천문학 연구, 강령학 연구, 금속학 연구, 빛무리, 대형 프리즘, 성운, 준비 운동, 별빛, 얼음 방패, 4연격, 전략가, 낡은 노심, 판금 갑옷, 소거법, 무기 연마, 방어구 연마, 질주, 퀵스텝, 진압, 별의 방주, 응수, 묵직한 한 방, 치환 합금, 청동 철퇴, 철벽, 전투 교본, 거울상, 가호.
...
흑요석 단검, 강철심장, 연사, 전술가, 마도서, 초신성, 유성우, 대분배, 견고한 태세, 법학 연구, 경제학 연구, 광학 연구, 광행시간, 늑대 부적, 거북이 부적, 오딘의 창.

- 경제학 연구: 2코스트 룰. 활성화된 장마다 에너지 사용 하한이 -3씩 내려간다.
- 광학 연구: 1코스트 룰. 매 플레이어 턴 시작 시 활성화된 장마다 광채 1장을 손패에 생성한다.
- 광행시간: 1코스트. 다음 턴 시작 시 광채 2장을 손패에 생성한다.
- 늑대 부적: 사용 불가. 인벤토리나 어떤 덱에든 가지고 있는 동안 힘 1을 얻는다. 중복 효과는 적용하지 않는다.
- 거북이 부적: 사용 불가. 인벤토리나 어떤 덱에든 가지고 있는 동안 강인함 1을 얻는다. 중복 효과는 적용하지 않는다.
...
발견 덱의 에디션은 현재 덱 점수 생성 규칙으로 정한다. 남은 점수 이하이고 충돌하지 않는 에디션만 후보가 되며, 후보별 선택 weight는 `에디션 점수^1.2`다. 같은 에디션은 중복되지 않으며 `재활용`과 `재활용+`는 함께 생성되지 않는다.

| 에디션 | 점수 | 효과 |
|---|---:|---|
| 별++ | 20 | 전투 시작 ★+2 |
| 추가 파일 | 10 | 빈 파일 1개 추가 |
| 아드레날린 | 50 | 전투 시작 아드레날린 카드 획득 |
| 압축 | 60 | 파일당 4장 배치 |
| 프리뷰 | 5 | 파일 생성 시 첫 번째 파일의 모든 카드를 앞면으로 배치 |
| 희귀 감지 | 5 | 모든 희귀 카드를 앞면으로 배치 |
| 방호 태세 | 5 | 전투 시작 시 방어 5 획득 |
| 지속 에너지 | 50 | 최대 에너지 +1 |
| 탐욕 | 20 | 전투 보상 골드 2배 |
| 재활용 | 10 | 턴 종료 때 남은 에너지를 ★로 전환 |
| 드로우+ | 10 | 전투 시작 시 무작위 파일에서 1장 드로우 |
| 별+ | 10 | 전투 시작 시 ★+1 |
| 에너지+ | 20 | 전투 시작 시 에너지+1 |
| 화력 | 20 | 전투 시작 시 힘 +2 |
| 성장 | 35 | 매 턴 시작 시 힘 +1 |
| 저항 | 30 | 전투 시작 시 물리 저항 1·마법 저항 1 |
| 거인 | 50 | 전투 시작 시 힘 +3·강인함 +3 |
| 여백의 미 | 30 | 현재 빈 파일 하나당 힘 +2 |
| 무적 | 100 | 전투 시작 시 무적 1 |
| 망치질 | 30 | 재련 시 ★ 획득 |
| 하이랜더 | 30 | 중복 카드 없이 덱이 가득 차 있으면 최대 에너지 +1 |
| 지속 드로우 | 40 | 매 턴 시작 시 무작위 파일에서 1장 드로우 |
| 재활용+ | 40 | 턴 종료 때 남은 에너지를 ★★로 전환 |
| 별+++++ | 50 | 전투 시작 시 ★+5 |
| 에너지+++ | 50 | 전투 시작 시 에너지+3 |
...
일반 전투에서 나오는 발견 덱은 보스가 아닌 적의 지역 번호 `n`에 대해 시작 점수 `n×30 + 정수(-5~+5)`를 정하고, 디버그 점수 덱 생성기와 같은 규칙으로 생성된다. 덱 케이스가 나오지 않으면 기존 카드 보상·티켓 보상 규칙을 따른다.
...
- 디버그 점수 덱 생성기는 시작 점수와 시도 횟수를 입력받는다. 희귀 카드 수의 목표 평균을 `μ=시작 점수 × 1/100 × min((100+시작 점수)/130, 2)`로 두고, 기대 시행 횟수 `5μ`인 랜덤화 이항분포를 사용한다. 시행 횟수는 `floor(5μ)` 또는 `floor(5μ)+1`을 선택해 평균적으로 `5μ`가 되게 하며, 각 시행의 성공확률은 `1/5`이다. 용량 증가 횟수는 목표 평균 `μ=시작 점수/30`을 유지하는 일반화 푸아송분포(`θ=-0.5`, `λ=(1-θ)μ=시작 점수/20`)로 정한다. 희귀 카드는 장당 8점, 용량 5 증가마다 13점이며, 에디션은 남은 점수 이하인 후보 중 `에디션 점수^1.2` 가중치 룰렛으로 추가한다. 에디션에는 누진세가 없다. 희귀·용량 점수를 뺀 남은 점수는 -10까지 허용하며 -11 이하면 폐기한다. 일반 전투의 보스가 아닌 적이 드롭하는 덱은 지역 번호 `n`의 시작 점수 `n×30 + 정수(-5~+5)`를 사용한다.
- 특별 카드는 생성 후 덱 점수에 포함하지 않는다. 남은 칸은 빈칸 3/8, 시작 카드 1/8, 일반 카드 1/4, 특별 카드 1/4 비율의 8-bag으로 슬롯 종류와 개수를 먼저 정한다. 이후 시작·일반 카드의 실제 종류는 일반 균등 복원추출보다 고유 카드 수가 평균 2/3이 되도록 응집 복원추출하며, 특별 카드는 기존처럼 균등하게 뽑는다.
- 텔레메트리는 런, 전투, 턴, 가한·받은 피해, 카드 사용, 카드 획득 경로, 적별 피해, 덱 스냅샷을 기록한다.
- 현재 런의 기록은 브라우저 `localStorage`에 저장되어 새로고침 후에도 이어지며, 새 런 시작 시 초기화된다.
