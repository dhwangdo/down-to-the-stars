...
- 플레이어 중심의 5×5, 총 25칸 안에서만 적의 위치와 상태를 실시간으로 표시한다. 이미 방문한 길 주변도 지도에 남지만, 시야 밖의 칸은 옅은 회색으로 표시되고 적 정보는 보이지 않는다.
...
- 적은 처음에 `Zzz` 상태로 생성된다.
- 각 지역의 적은 3% 확률로 바로 다음 지역의 적 풀에서 대신 생성된다. 다음 지역 적 풀이 아직 없으면 현재 지역 적 풀을 사용한다.
- 적 이름과 현재 상태는 시야 안에서 항상 표시된다.
...
- 5×5 범위에서 가운데 십자 5칸이 내부이며, 나머지는 `ㅁ@@@ㅁ / @@ㅁ@@ / @ㅁㅁㅁ@ / @@ㅁ@@ / ㅁ@@@ㅁ` 형태로 단단한 돌과 바깥 칸이 배치된다.
- 내부에 들어가면 가운데 십자 5칸과 이를 감싸는 단단한 돌 16칸만 보인다. 폭탄 등으로 돌 칸에 올라가면 외부 판정으로 돌아가 일반 지도 시야가 다시 보인다.
- 밖에서는 안전 지역과 2칸 이내일 때만 안전 지역 구조가 보인다.
...
전투 승리 시 쓰러뜨린 적의 원래 지역 번호가 `n`이면 골드는 `1`부터 `n` 사이에서 나온다. 낮은 지역에 희귀 출현한 상위 지역 적도 상위 지역 기준 보상을 준다.
