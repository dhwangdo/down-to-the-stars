...
- 특별 카드는 생성 후 덱 점수에 포함하지 않는다. 남은 칸은 빈칸 1/4, 시작 카드 1/8, 일반 카드 3/8, 특별 카드 1/4 비율의 8-bag으로 슬롯 종류와 개수를 먼저 정한다. 이후 시작·일반 카드의 실제 종류는 일반 균등 복원추출보다 고유 카드 수가 평균 2/3이 되도록 응집 복원추출하며, 특별 카드는 기존처럼 균등하게 뽑는다.
