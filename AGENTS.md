...
- 최신 원본 작업공간은 `C:\Users\kdh03\Documents\Codex\2026-07-25\solitaire-deckbattle-enemies\solitaire-deckbattle-prototype`이다.
- 작업 시작 전 실제 현재 경로와 `rtk git status`를 확인한다.
- `.codex/worktrees` 아래의 별도 worktree에는 원본 작업공간의 미커밋 변경이 자동으로 포함되지 않는다. 최신 상태라고 추측하지 않는다.
- 별도 worktree에서 작업해야 한다면 원본과의 차이를 먼저 확인한다. 구버전 파일을 원본에 통째로 덮어쓰거나 병합하지 않는다.
- 질문·토론 전용 작업에서는 사용자가 명시적으로 구현을 요청하지 않는 한 코드 수정, 서버 실행, 커밋 또는 병합을 하지 않는다.
- 개발 서버 실행 전 포트 점유 상태를 확인한다. 실행 후에는 실행한 작업공간의 절대 경로와 실제 접속 포트를 사용자에게 보고한다.
- `localhost:3000`이 최신 서버라고 가정하지 않는다. 포트 번호만으로 코드 버전을 판정하지 않는다.
- 여러 DTTS 작업이 동시에 실행될 때는 서로 다른 포트를 사용하고, 작업 종료 시 자신이 실행한 서버를 종료한다.
- 다른 작업공간의 변경을 가져오기 전 최신 원본의 미커밋 변경과 충돌하거나 기능을 되돌릴 가능성을 검토하고 사용자에게 알린다.
...
- 원격 저장소: `https://github.com/dhwangdo/down-to-the-stars`
- 공개 주소: `https://dhwangdo.github.io/down-to-the-stars/`
